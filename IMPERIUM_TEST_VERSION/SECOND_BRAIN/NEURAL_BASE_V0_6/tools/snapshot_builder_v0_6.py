"""
Second Brain Neural Base V0.6 — Snapshot Builder
Reads zone_registry + truth_matrix, evaluates health per zone,
collects telemetry (including task_packages), writes neural_snapshot_live.json.
"""
...
V06_ROOT         = os.path.dirname(TOOLS_DIR)
...
TRUTH_DIR        = os.path.join(V06_ROOT, "truth_matrix")
REPORTS_DIR      = os.path.join(V06_ROOT, "reports")

ZONE_REGISTRY    = os.path.join(REGISTRY_DIR, "zone_registry_v0_6.json")
...
def resolve_path(pattern):
    return os.path.join(REPO_ROOT, pattern.replace("/", os.sep))
...
def collect_telemetry_for_zone(zone_id, zone_def):
    ...
    if zone_id == "task_intake":
        ...
        if d:
            ...
            launched = sum(1 for t in tasks if t.get("status") == "TASK_READY_FOR_SERVITOR")
            tel["launched_task_count"] = launched
            if tasks:
                try:
        ...
        # task_packages count
        pkg_dir = resolve_path("IMPERIUM_TEST_VERSION/SECOND_BRAIN/RUNTIME/task_packages")
        if os.path.isdir(pkg_dir):
            pkgs = [d for d in os.listdir(pkg_dir) if os.path.isdir(os.path.join(pkg_dir, d))]
            tel["task_package_count"] = len(pkgs)
            # latest verdict
            verdicts = []
            for pkg in pkgs:
                vr, _ = load_json_safe(os.path.join(pkg_dir, pkg, "validation_report.json"))
                if vr:
                    verdicts.append(vr.get("verdict", "UNKNOWN"))
            if verdicts:
                tel["latest_package_verdict"] = verdicts[-1]
        else:
            tel["task_package_count"] = 0
        ...
        pkg_dir = resolve_path("IMPERIUM_TEST_VERSION/SECOND_BRAIN/RUNTIME/task_packages")
        if os.path.isdir(pkg_dir):
            tel["task_package_count"] = len([d for d in os.listdir(pkg_dir) if os.path.isdir(os.path.join(pkg_dir, d))])
...
def build_snapshot():
    ...
    print("=" * 60)
    print("Second Brain V0.6 — Snapshot Builder")
    print("=" * 60)

    registry, err = load_json_safe(ZONE_REGISTRY)
    if err:
        print(f"[FATAL] Cannot load zone_registry_v0_6.json: {err}")
        sys.exit(1)
    ...
    for zone_def in zones:
        ...
        if terr:
            print(f"  [WARN] No truth contract for {zone_id}")
        ...
        zone_snapshot = {
            ...
            "corridor_mode": zone_def.get("corridor_mode"),
    ...
    pkg_dir = resolve_path("IMPERIUM_TEST_VERSION/SECOND_BRAIN/RUNTIME/task_packages")
    task_package_count = len([d for d in os.listdir(pkg_dir) if os.path.isdir(os.path.join(pkg_dir, d))]) if os.path.isdir(pkg_dir) else 0

    strands = layout.get("strands", []) if layout else []

    snapshot = {
        "snapshot_id": f"NBV06-SNAP-{datetime.datetime.utcnow().strftime('%Y%m%d%H%M%S')}",
        "truth_lock_run_id": f"TRUTHLOCK-V06-{datetime.datetime.utcnow().strftime('%Y%m%d%H%M%S')}",
        "timestamp_utc": now_iso(),
        "schema_version": "neural_snapshot.v0.6",
        ...
        "task_package_count": task_package_count,
        "telemetry": {
    ...
    print(f"Task packages:   {task_package_count}")
