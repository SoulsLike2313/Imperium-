...
$script:SyntheticFullRunPath = "E:\IMPERIUM\TOOLS\astronomicon_synthetic_full_run_v0_1.ps1"
...
function Set-ButtonVisual {
    ...
    switch ($State) {
        "WORKING" {
            $Button.Background = [System.Windows.Media.Brushes]::DarkGoldenrod
            $Button.BorderBrush = [System.Windows.Media.Brushes]::Gold
        }
        "DONE" {
            $Button.Background = [System.Windows.Media.Brushes]::SeaGreen
            $Button.BorderBrush = [System.Windows.Media.Brushes]::MediumSeaGreen
        }
        "ERROR" {
            $Button.Background = [System.Windows.Media.Brushes]::Firebrick
...
function Set-AliveState {
    ...
    $script:IsBusy = $Busy
    if ($Busy) {
        $script:txtAlive.Text = "TASK IS ALIVE :: $Label"
        $script:txtAlive.Foreground = [System.Windows.Media.Brushes]::Gold
    }
    else {
        $script:txtAlive.Text = "TASK IS ALIVE :: IDLE"
        $script:txtAlive.Foreground = [System.Windows.Media.Brushes]::LightGreen
    }
}
...
function Set-ActiveLayer {
    ...
    foreach ($key in $script:LayerMap.Keys) {
        $border = $script:LayerMap[$key]
        $border.BorderThickness = "1"
        $border.BorderBrush = [System.Windows.Media.Brushes]::SlateGray
    }

    if ($script:LayerMap.ContainsKey($LayerKey)) {
        $activeBorder = $script:LayerMap[$LayerKey]
        $activeBorder.BorderThickness = "2"
        $activeBorder.BorderBrush = [System.Windows.Media.Brushes]::DeepSkyBlue
    }
}
...
function Invoke-UiAction {
    param(
        [Parameter(Mandatory = $true)][System.Windows.Controls.Button]$Button,
        [Parameter(Mandatory = $true)][string]$LayerKey,
        [Parameter(Mandatory = $true)][string]$AliveLabel,
        [Parameter(Mandatory = $true)][scriptblock]$Action
    )

    Set-ButtonVisual -Button $Button -State "WORKING"
    Set-ActiveLayer -LayerKey $LayerKey
    Set-AliveState -Busy $true -Label $AliveLabel
    try {
        & $Action
        Set-ButtonVisual -Button $Button -State "DONE"
    }
    catch {
        Set-ButtonVisual -Button $Button -State "ERROR"
        throw
    }
    finally {
        Set-AliveState -Busy $false -Label "IDLE"
    }
}
...
function New-DefaultTaskId {
    ...
    return "GTASK-$stamp-ASTRONOMICON-DASHBOARD-V0_7"
}
...
"@
    ...
    $dlgTitle.Text = "Astronomicon General Task v0.7"
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
        Title="IMPERIUM :: ASTRONOMICON DASHBOARD v0.7"
        Height="1000"
        Width="1700"
        WindowStartupLocation="CenterScreen"
        Background="#070B15"
    ...
    <Grid Margin="12">
        ...
        <Border Grid.Column="0" CornerRadius="18" Background="#0D1426" BorderBrush="#273858" BorderThickness="1" Margin="0,0,10,0">
            <DockPanel Margin="16">
                <StackPanel DockPanel.Dock="Top">
                    ...
                    <TextBlock Text="Pipeline forge v0.7" Foreground="#8DA7FF" Margin="0,4,0,16"/>
                    ...
                    <Button Name="btnSyntheticFullRun" Content="10) Synthetic Full Run" Background="#735214" BorderBrush="#C08A22"/>
        ...
        <Border Grid.Column="1" CornerRadius="18" Background="#0A1020" BorderBrush="#273858" BorderThickness="1" Margin="0,0,10,0">
            <Grid Margin="18">
                ...
                <Border Grid.Row="0" CornerRadius="12" Background="#111C30" BorderBrush="#35506E" BorderThickness="1" Padding="10" Margin="0,0,0,10">
                    <TextBlock Name="txtAlive" Text="TASK IS ALIVE :: IDLE" Foreground="#A5FFB8" FontWeight="Bold" FontFamily="Consolas"/>
                </Border>
                ...
                <Border Name="brLayerGeneral" Grid.Row="2" CornerRadius="24" Background="#101A31" BorderBrush="#576DFF" BorderThickness="2" Padding="16" Margin="80,0,80,12">
                    <StackPanel>
                ...
                <Grid Grid.Row="3" Margin="0,0,0,12">
                    ...
                    <Border Name="brLayerLocal" Grid.Column="0" CornerRadius="16" Background="#0E172C" BorderBrush="#3A4F87" BorderThickness="1" Padding="10" Margin="0,0,6,0">
                        <DockPanel>
                    ...
                    <Border Name="brLayerStages" Grid.Column="1" CornerRadius="16" Background="#0E172C" BorderBrush="#33405E" BorderThickness="1" Padding="10" Margin="6,0,0,0">
                        <DockPanel>
                ...
                <Border Name="brLayerGit" Grid.Row="4" CornerRadius="16" Background="#111827" BorderBrush="#5A3DF0" BorderThickness="1" Padding="10" Margin="0,0,0,12">
                    <DockPanel>
                ...
                <Border Grid.Row="5" CornerRadius="14" Background="#0E172C" BorderBrush="#26395E" BorderThickness="1" Padding="12">
                    <Grid>
                        <Grid.ColumnDefinitions>
                            <ColumnDefinition Width="*"/>
                            <ColumnDefinition Width="*"/>
...
$script:txtAlive = $window.FindName("txtAlive")
...
$script:LayerMap = @{
    general = $window.FindName("brLayerGeneral")
    local = $window.FindName("brLayerLocal")
    stages = $window.FindName("brLayerStages")
    git = $window.FindName("brLayerGit")
}
...
$btnSyntheticFullRun = $window.FindName("btnSyntheticFullRun")
...
Refresh-PipelinePanel
Set-AliveState -Busy $false -Label "IDLE"
Set-ActiveLayer -LayerKey "general"

$script:PulseTimer = New-Object System.Windows.Threading.DispatcherTimer
$script:PulseTimer.Interval = [TimeSpan]::FromMilliseconds(350)
$script:PulseTimer.Add_Tick({
    if (-not $script:txtAlive) { return }
    if ($script:IsBusy) {
        $script:PulseTick++
        if (($script:PulseTick % 2) -eq 0) {
            $script:txtAlive.Foreground = [System.Windows.Media.Brushes]::Gold
        }
        else {
            $script:txtAlive.Foreground = [System.Windows.Media.Brushes]::Orange
        }
    }
    else {
        $script:txtAlive.Foreground = [System.Windows.Media.Brushes]::LightGreen
...
$btnCreateGeneral.Add_Click({
    try {
        Invoke-UiAction -Button $btnCreateGeneral -LayerKey "general" -AliveLabel "CREATE GENERAL TASK" -Action {
...
$btnParse.Add_Click({
    try {
        Invoke-UiAction -Button $btnParse -LayerKey "local" -AliveLabel "PARSE PIPELINE" -Action {
            if (-not $script:CurrentInputPath -or -not (Test-Path -LiteralPath $script:CurrentInputPath)) {
...
$btnExportLocalSpeculum.Add_Click({
    try {
        Invoke-UiAction -Button $btnExportLocalSpeculum -LayerKey "local" -AliveLabel "EXPORT LOCAL REVIEW" -Action {
...
$btnImportLocalSpeculum.Add_Click({
    try {
        Invoke-UiAction -Button $btnImportLocalSpeculum -LayerKey "local" -AliveLabel "IMPORT LOCAL REFINEMENTS" -Action {
...
$btnDecomposeSelected.Add_Click({
    try {
        Invoke-UiAction -Button $btnDecomposeSelected -LayerKey "stages" -AliveLabel "DECOMPOSE TO STAGES" -Action {
            if (-not $script:CurrentOutputRoot) { throw "No output root found." }
...
$btnExportStageSpeculum.Add_Click({
    try {
        Invoke-UiAction -Button $btnExportStageSpeculum -LayerKey "stages" -AliveLabel "EXPORT STAGE REVIEW" -Action {
            if (-not $script:CurrentOutputRoot) { throw "No output root found." }
            if ($null -eq $script:lstLocalTasks.SelectedItem) { throw "Select one Local Task first." }
...
$btnImportStageSpeculum.Add_Click({
    try {
        Invoke-UiAction -Button $btnImportStageSpeculum -LayerKey "stages" -AliveLabel "IMPORT STAGE REFINEMENTS" -Action {
            if (-not $script:CurrentOutputRoot) { throw "No output root found." }
            if ($null -eq $script:lstLocalTasks.SelectedItem) { throw "Select one Local Task first." }
...
$btnCommitTaskPlan.Add_Click({
    try {
        Invoke-UiAction -Button $btnCommitTaskPlan -LayerKey "git" -AliveLabel "COMMIT TASK PLAN" -Action {
            if (-not $script:CurrentTaskRoot -or -not (Test-Path -LiteralPath $script:CurrentTaskRoot)) {
...
$btnPushTaskPlan.Add_Click({
    try {
        Invoke-UiAction -Button $btnPushTaskPlan -LayerKey "git" -AliveLabel "PUSH TASK PLAN" -Action {
...
$btnSyntheticFullRun.Add_Click({
    try {
        Invoke-UiAction -Button $btnSyntheticFullRun -LayerKey "git" -AliveLabel "SYNTHETIC FULL RUN" -Action {
            $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
            $syntheticOutput = Join-Path $script:RepoRoot "ARTIFACTS\ASTRONOMICON_SYNTHETIC_RUNS\$stamp"
            New-Item -ItemType Directory -Force -Path $syntheticOutput | Out-Null
            Status -State "WORKING" -Message "Starting synthetic full-run for three General Tasks."
            Invoke-BackendScript -ScriptPath $script:SyntheticFullRunPath -Parameters @(
                "-RepoRoot", $script:RepoRoot,
                "-GeneralTasksRoot", $script:GeneralTasksRoot,
                "-SyntheticPrefix", "GTASK-SYNTH-V0_7",
                "-OutputRoot", $syntheticOutput,
                "-TaskCount", "3"
            )
            $summaryPath = Join-Path $syntheticOutput "SYNTHETIC_SUMMARY.json"
            Show-PreviewFile -Path $summaryPath
            $summary = Get-Content -LiteralPath $summaryPath -Encoding UTF8 -Raw | ConvertFrom-Json
            if ([int]$summary.errors -gt 0) {
                Status -State "FAIL" -Message "Synthetic run finished with errors. See summary."
            }
            elseif ([int]$summary.blocked_states -gt 0) {
                Status -State "BLOCKED" -Message "Synthetic run finished with blocked states or orphans."
            }
            else {
                Status -State "PASS" -Message "Synthetic run completed clean."
            }
        }
    }
    catch {
        Status -State "FAIL" -Message "Synthetic full run failed: $($_.Exception.Message)"
    }
})
...
$script:lstLocalTasks.Add_SelectionChanged({
    try {
        ...
        Set-ActiveLayer -LayerKey "local"
        $localTaskId = [string]$script:lstLocalTasks.SelectedItem
        Refresh-StagesForSelectedLocalTask
...
$script:lstStages.Add_SelectionChanged({
    try {
        ...
        if ($null -eq $script:lstLocalTasks.SelectedItem) { return }
        Set-ActiveLayer -LayerKey "stages"
        $localTaskId = [string]$script:lstLocalTasks.SelectedItem
...
$window.Add_ContentRendered({
    Status -State "READY" -Message "Dashboard v0.7 ready."
})
