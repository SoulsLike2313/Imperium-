let DATA = null;
let lastFrame = performance.now();
let fps = 60;

const $ = (id) => document.getElementById(id);

function showToast(text) {
  const t = $("toast");
  t.textContent = text;
  t.classList.remove("hidden");
  setTimeout(() => t.classList.add("hidden"), 3200);
}

async function loadData() {
  const res = await fetch("/api/data?ts=" + Date.now());
  DATA = await res.json(); window.DATA = DATA;
  renderAll();
}

async function runRefresh() {
  showToast("Running Doctrinarium refresh...");
  const res = await fetch("/api/refresh", { method: "POST" });
  const data = await res.json();
  if (data.ok) showToast("Refresh complete");
  else showToast("Refresh failed: " + (data.error || data.exit_code));
  await loadData();
}

async function openTarget(target) {
  await fetch("/api/open?target=" + encodeURIComponent(target), { method: "POST" });
}

function setText(id, value) {
  const el = $(id);
  if (el) el.textContent = value;
}

function pct(n, d) {
  if (!d || d <= 0) return 0;
  return Math.max(0, Math.min(100, Math.round((n / d) * 100)));
}

function setBar(id, p) {
  const el = $(id);
  if (el) el.style.width = p + "%";
}

function ring(id, p) {
  const c = $(id);
  if (!c) return;
  const circ = 2 * Math.PI * 48;
  c.style.strokeDasharray = circ;
  c.style.strokeDashoffset = circ - (circ * p / 100);
}

function severityClass(blockers) {
  if (blockers >= 9) return "critical";
  if (blockers >= 4) return "risk";
  return "ok";
}

function boolMark(v) {
  return v ? "OK" : "MISS";
}

function renderAll() {
  const s = DATA.status || {};
  const g = DATA.gaps || {};
  const u = DATA.utility || {};
  const laws = DATA.laws || {};
  const paths = DATA.paths || {};
  const useful = DATA.useful || {};

  const totalLaws = s?.law_registry_status?.total_laws || 0;
  const notEnforced = s?.law_registry_status?.not_fully_enforced_count || 0;
  const hardOpen = s?.law_registry_status?.hard_not_fully_enforced_count || 0;

  const organsChecked = g.total_organs_checked || 0;
  const blockers = g.total_blockers_found || 0;

  const uBacked = u?.summary?.script_backed_count || 0;
  const uDeclared = u?.summary?.utility_declared_count || 0;
  const uWarnings = u?.summary?.warnings_count || 0;

  setText("mRealTask", s.real_task_execution_allowed ? "ALLOWED" : "BLOCKED");
  setText("mBootstrap", s.bootstrap_review_allowed ? "ALLOWED" : "TRUE");
  setText("mDoctrine", "OWNER REVIEW");
  setText("mLaws", `${notEnforced} / ${totalLaws}`);
  setText("mOrgans", `${organsChecked} / ${organsChecked}`);
  setText("mBlockers", blockers);
  setText("mUtilities", `${uBacked} / ${uDeclared || 0}`);

  setText("orbText", s.real_task_execution_allowed ? "PASS" : "BLOCKED");

  const doctrinePct = (s.passport_status && s.constitution_status && s.codex_status) ? 68 : 0;
  const lawsPct = totalLaws ? 100 - pct(notEnforced, totalLaws) : 0;
  const organPct = organsChecked ? Math.max(0, 100 - Math.min(100, blockers * 2)) : 0;
  const utilityPct = pct(uBacked, organsChecked);

  setBar("barDoctrine", doctrinePct);
  setBar("barLaws", lawsPct);
  setBar("barOrgans", organPct);
  setBar("barUtility", utilityPct);
  setText("pctDoctrine", doctrinePct + "%");
  setText("pctLaws", lawsPct + "%");
  setText("pctOrgans", organPct + "%");
  setText("pctUtility", utilityPct + "%");

  const healthy = (g.organs || []).filter(o => (o.blockers || []).length === 0).length;
  const risk = (g.organs || []).filter(o => (o.blockers || []).length > 0 && (o.blockers || []).length < 9).length;
  const critical = (g.organs || []).filter(o => (o.blockers || []).length >= 9).length;
  const health = pct(healthy, organsChecked);

  setText("healthPct", health + "%");
  setText("healthGood", healthy);
  setText("healthRisk", risk);
  setText("healthCritical", critical);
  setText("healthVerdict", g.verdict || "unknown");
  setText("statusTotal", `${organsChecked} total`);

  $("lineGood").style.width = pct(healthy, organsChecked) + "%";
  $("lineRisk").style.width = pct(risk, organsChecked) + "%";
  $("lineCritical").style.width = pct(critical, organsChecked) + "%";
  ring("healthRing", health);

  setText("utilityVerdict", u.verdict || "unknown");
  setText("utilityPct", utilityPct + "%");
  setText("uDeclared", uDeclared);
  setText("uBacked", uBacked);
  setText("uWarnings", uWarnings);
  ring("utilityRing", utilityPct);

  setText("lTotal", totalLaws);
  setText("lOpen", notEnforced);
  setText("lHard", hardOpen);
  setText("lawZero", (100 - pct(notEnforced, totalLaws)) + "%");

  renderTopOrganRows(g.organs || []);
  renderOrganCards(g.organs || []);
  renderGapList(g.major_gaps || []);
  renderUtilityCards(u.organs || []);
  renderLawList(laws);
  renderPaths(paths);
  renderPriorityFixes(useful.priority_fixes || []);
  renderRiskMatrix(useful.organ_risk || []);
  drawStatusDonut(g);
  drawRadar(g);
  drawTrend();
}

function renderTopOrganRows(organs) {
  const tb = $("topOrganRows");
  tb.innerHTML = "";
  const sorted = [...organs].sort((a,b) => (b.blockers || []).length - (a.blockers || []).length).slice(0, 7);
  for (const o of sorted) {
    const blockers = (o.blockers || []).length;
    const tr = document.createElement("tr");
    const topGap = (o.blockers || [])[0] || "-";
    tr.innerHTML = `
      <td>${o.organ_id}</td>
      <td><span class="badge ${severityClass(blockers)}">${o.classification}</span></td>
      <td>${blockers}</td>
      <td>${boolMark(o.organ_contract_exists)}</td>
      <td>${boolMark(o.self_report_exists)}</td>
      <td>${boolMark(o.receipts_exists)}</td>
      <td>${topGap}</td>
    `;
    tb.appendChild(tr);
  }
}

function renderOrganCards(organs) {
  const root = $("organCards");
  root.innerHTML = "";
  for (const o of organs) {
    const blockers = (o.blockers || []).length;
    const div = document.createElement("div");
    div.className = "organ-card";
    div.innerHTML = `
      <h3>${o.organ_id} <span class="badge ${severityClass(blockers)}">${o.classification}</span></h3>
      <div class="meta">
        <div>Path: ${o.path}</div>
        <div>Blockers: ${blockers}</div>
        <div>Contract: ${boolMark(o.organ_contract_exists)}</div>
        <div>Self-report: ${boolMark(o.self_report_exists)}</div>
        <div>Receipts: ${boolMark(o.receipts_exists)}</div>
        <div>Why not canon: ${(o.why_not_canon || []).join("; ") || "-"}</div>
      </div>
    `;
    root.appendChild(div);
  }
}

function renderGapList(gaps) {
  $("gapCountLabel").textContent = `${gaps.length} gaps`;
  const root = $("gapList");
  root.innerHTML = "";
  for (const gap of gaps) {
    const div = document.createElement("div");
    div.className = "gap-card";
    div.innerHTML = `<h3>${gap.organ}</h3><p>${gap.gap}</p>`;
    root.appendChild(div);
  }
}

function renderUtilityCards(organs) {
  const root = $("utilityCards");
  root.innerHTML = "";
  for (const o of organs) {
    const warnings = [...(o.warnings || []), ...(o.blockers || [])];
    const div = document.createElement("div");
    div.className = "organ-card";
    div.innerHTML = `
      <h3>${o.organ_id} <span class="badge ${o.script_backed ? "ok" : "risk"}">${o.script_backed ? "script-backed" : "not backed"}</span></h3>
      <div class="meta">
        <div>Status: ${o.organ_status}</div>
        <div>Utility declared: ${o.utility_declared}</div>
        <div>Utility path: ${o.utility_path || "-"}</div>
        <div>Warnings: ${warnings.join("; ") || "-"}</div>
      </div>
    `;
    root.appendChild(div);
  }
}

function getLawArray(laws) {
  if (!laws) return [];
  if (Array.isArray(laws.laws)) return laws.laws;
  if (Array.isArray(laws.mandatory_laws)) return laws.mandatory_laws;
  if (Array.isArray(laws.entries)) return laws.entries;
  return [];
}

function renderLawList(laws) {
  const arr = getLawArray(laws);
  $("lawCountLabel").textContent = `${arr.length} laws`;
  const root = $("lawList");
  root.innerHTML = "";
  for (const law of arr) {
    const div = document.createElement("div");
    div.className = "law-card";
    div.innerHTML = `
      <h3><span class="law-id">${law.law_id || "LAW"}</span> - ${law.title || ""}</h3>
      <div class="meta">
        <div>Severity: ${law.severity || "-"}</div>
        <div>Status: ${law.status || "-"}</div>
        <div>Enforcement: ${law.enforcement_status || "-"}</div>
        <div>Verdict: ${law.violation_verdict || "-"}</div>
        <div>Source: ${law.source_document_path || "-"}</div>
      </div>
    `;
    root.appendChild(div);
  }
}

function renderPaths(paths) {
  $("pathsText").textContent = Object.entries(paths || {}).map(([k,v]) => `${k}:\n  ${v}`).join("\n\n");
}

function drawStatusDonut(g) {
  const canvas = $("statusDonut");
  const ctx = canvas.getContext("2d");
  const r = canvas.getBoundingClientRect();
  canvas.width = r.width * devicePixelRatio;
  canvas.height = r.height * devicePixelRatio;
  ctx.scale(devicePixelRatio, devicePixelRatio);
  ctx.clearRect(0,0,r.width,r.height);

  const summary = g.classification_summary || {};
  const entries = Object.entries(summary);
  const total = entries.reduce((s, [,v]) => s + Number(v), 0) || 1;
  const colors = ["#45d483", "#ffd45a", "#ff4f6d", "#b45cff", "#53a7ff"];

  let start = -Math.PI/2;
  const cx = r.width/2, cy = r.height/2, rad = Math.min(r.width,r.height)/2 - 18;

  entries.forEach(([key,val], i) => {
    const a = (Number(val)/total) * Math.PI * 2;
    ctx.beginPath();
    ctx.arc(cx, cy, rad, start, start+a);
    ctx.lineWidth = 28;
    ctx.strokeStyle = colors[i % colors.length];
    ctx.stroke();
    start += a;
  });

  ctx.fillStyle = "#edf2ff";
  ctx.font = "bold 24px Segoe UI";
  ctx.textAlign = "center";
  ctx.fillText(total, cx, cy);
  ctx.font = "11px Segoe UI";
  ctx.fillStyle = "#9ca8c3";
  ctx.fillText("TOTAL", cx, cy + 20);

  const legend = $("statusLegend");
  legend.innerHTML = "";
  entries.forEach(([key,val], i) => {
    const div = document.createElement("div");
    div.innerHTML = `<span><i style="background:${colors[i % colors.length]}"></i>${key}</span><b>${val}</b>`;
    legend.appendChild(div);
  });
}

function drawRadar(g) {
  const canvas = $("radarCanvas");
  const ctx = canvas.getContext("2d");
  const r = canvas.getBoundingClientRect();
  canvas.width = r.width * devicePixelRatio;
  canvas.height = r.height * devicePixelRatio;
  ctx.scale(devicePixelRatio, devicePixelRatio);
  ctx.clearRect(0,0,r.width,r.height);

  const labels = ["FOLDER", "UNKNOWN", "SCAFFOLD", "CONTRACT", "GAPS"];
  const summary = g.classification_summary || {};
  const vals = [
    summary.FOLDER_ONLY || 0,
    summary.UNKNOWN || 0,
    summary.SCAFFOLD || 0,
    (g.major_gaps || []).filter(x => String(x.gap).includes("ORGAN_CONTRACT")).length,
    Math.min(10, Math.round((g.total_blockers_found || 0)/6))
  ];
  const max = Math.max(1, ...vals);
  const cx = r.width/2, cy = r.height/2 + 10;
  const rad = Math.min(r.width, r.height)/2 - 35;

  ctx.strokeStyle = "rgba(255,255,255,.12)";
  ctx.fillStyle = "rgba(180,92,255,.16)";
  for (let layer=1; layer<=4; layer++) {
    ctx.beginPath();
    for (let i=0; i<labels.length; i++) {
      const a = -Math.PI/2 + (i/labels.length)*Math.PI*2;
      const rr = rad * layer/4;
      const x = cx + Math.cos(a)*rr, y = cy + Math.sin(a)*rr;
      if (i===0) ctx.moveTo(x,y); else ctx.lineTo(x,y);
    }
    ctx.closePath(); ctx.stroke();
  }

  ctx.beginPath();
  vals.forEach((v,i) => {
    const a = -Math.PI/2 + (i/labels.length)*Math.PI*2;
    const rr = rad * v/max;
    const x = cx + Math.cos(a)*rr, y = cy + Math.sin(a)*rr;
    if (i===0) ctx.moveTo(x,y); else ctx.lineTo(x,y);
  });
  ctx.closePath();
  ctx.fillStyle = "rgba(180,92,255,.35)";
  ctx.fill();
  ctx.strokeStyle = "#ff4fd8";
  ctx.lineWidth = 2;
  ctx.stroke();

  ctx.fillStyle = "#9ca8c3";
  ctx.font = "11px Segoe UI";
  ctx.textAlign = "center";
  labels.forEach((label,i) => {
    const a = -Math.PI/2 + (i/labels.length)*Math.PI*2;
    ctx.fillText(label, cx + Math.cos(a)*(rad+22), cy + Math.sin(a)*(rad+22));
  });
}

function drawTrend() {
  const canvas = $("trendCanvas");
  const ctx = canvas.getContext("2d");
  const r = canvas.getBoundingClientRect();
  canvas.width = r.width * devicePixelRatio;
  canvas.height = r.height * devicePixelRatio;
  ctx.scale(devicePixelRatio, devicePixelRatio);
  ctx.clearRect(0,0,r.width,r.height);

  const pts = Array.from({length: 28}, (_,i) => {
    return 0.45 + Math.sin(i*0.55)*0.12 + Math.random()*0.08;
  });

  ctx.beginPath();
  pts.forEach((p,i) => {
    const x = (i/(pts.length-1))*r.width;
    const y = r.height - p*r.height;
    if (i===0) ctx.moveTo(x,y); else ctx.lineTo(x,y);
  });
  ctx.strokeStyle = "#b45cff";
  ctx.lineWidth = 2;
  ctx.stroke();

  const grad = ctx.createLinearGradient(0,0,0,r.height);
  grad.addColorStop(0, "rgba(180,92,255,.45)");
  grad.addColorStop(1, "rgba(180,92,255,0)");
  ctx.lineTo(r.width,r.height);
  ctx.lineTo(0,r.height);
  ctx.closePath();
  ctx.fillStyle = grad;
  ctx.fill();
}

function animateBg(now) {
  const canvas = $("bgCanvas");
  const ctx = canvas.getContext("2d");
  const w = canvas.width = window.innerWidth * devicePixelRatio;
  const h = canvas.height = window.innerHeight * devicePixelRatio;
  ctx.scale(devicePixelRatio, devicePixelRatio);

  const W = window.innerWidth, H = window.innerHeight;
  ctx.clearRect(0,0,W,H);

  for (let wave=0; wave<4; wave++) {
    ctx.beginPath();
    const yBase = H - 80 - wave*18;
    for (let x=0; x<=W; x+=8) {
      const y = yBase + Math.sin(x*0.012 + now*0.0015 + wave)*18 + Math.sin(x*0.027 + now*0.001)*8;
      if (x===0) ctx.moveTo(x,y); else ctx.lineTo(x,y);
    }
    ctx.strokeStyle = `rgba(180,92,255,${0.14 + wave*0.035})`;
    ctx.lineWidth = 1.4;
    ctx.shadowBlur = 16;
    ctx.shadowColor = "#b45cff";
    ctx.stroke();
  }

  for (let i=0; i<70; i++) {
    const x = (Math.sin(i*77.7 + now*0.0002)+1)/2 * W;
    const y = (Math.cos(i*31.1 + now*0.0003)+1)/2 * H;
    const a = 0.12 + (Math.sin(now*0.002 + i)+1)*0.12;
    ctx.fillStyle = `rgba(255,79,216,${a})`;
    ctx.fillRect(x, y, 1.4, 1.4);
  }

  const dt = now - lastFrame;
  lastFrame = now;
  fps = Math.round(1000 / Math.max(1, dt));
  $("fpsValue").textContent = Math.min(99, fps);

  requestAnimationFrame(animateBg);
}

function animateSpark(now) {
  const canvas = $("sparkCanvas");
  const ctx = canvas.getContext("2d");
  const r = canvas.getBoundingClientRect();
  canvas.width = r.width * devicePixelRatio;
  canvas.height = r.height * devicePixelRatio;
  ctx.scale(devicePixelRatio, devicePixelRatio);
  ctx.clearRect(0,0,r.width,r.height);
  ctx.beginPath();
  for (let x=0; x<r.width; x+=8) {
    const y = r.height/2 + Math.sin(x*0.04 + now*0.005)*7 + Math.sin(x*0.17 + now*0.002)*3;
    if (x===0) ctx.moveTo(x,y); else ctx.lineTo(x,y);
  }
  ctx.strokeStyle = "rgba(180,92,255,.72)";
  ctx.lineWidth = 1.5;
  ctx.shadowBlur = 10;
  ctx.shadowColor = "#b45cff";
  ctx.stroke();
  requestAnimationFrame(animateSpark);
}

document.querySelectorAll(".nav-item").forEach(btn => {
  btn.addEventListener("click", () => {
    document.querySelectorAll(".nav-item").forEach(b => b.classList.remove("active"));
    btn.classList.add("active");
    const view = btn.dataset.view;
    document.querySelectorAll(".view").forEach(v => v.classList.remove("active"));
    $(`view-${view}`).classList.add("active");
  });
});

document.querySelectorAll("[data-view-jump]").forEach(btn => {
  btn.addEventListener("click", () => {
    const v = btn.dataset.viewJump;
    document.querySelector(`.nav-item[data-view="${v}"]`)?.click();
  });
});

document.querySelectorAll("[data-open]").forEach(btn => {
  btn.addEventListener("click", () => openTarget(btn.dataset.open));
});

$("btnRefresh").addEventListener("click", runRefresh);
$("btnReload").addEventListener("click", loadData);

loadData().catch(err => showToast("Load failed: " + err.message));
requestAnimationFrame(animateBg);
requestAnimationFrame(animateSpark);

function renderPriorityFixes(items) {
  const root = $("priorityList");
  if (!root) return;

  root.innerHTML = "";

  if (!items || !items.length) {
    root.innerHTML = `<div class="gap-card"><h3>No priority fixes found</h3><p>Run refresh or check reports.</p></div>`;
    return;
  }

  items.forEach((it, idx) => {
    const div = document.createElement("div");
    div.className = "gap-card";
    div.innerHTML = `
      <h3>#${idx + 1} ${it.organ}</h3>
      <p><b>${it.action}</b></p>
      <p>${it.reason}</p>
    `;
    root.appendChild(div);
  });
}

function renderRiskMatrix(items) {
  const root = $("riskMatrix");
  if (!root) return;

  root.innerHTML = "";

  const sorted = [...(items || [])].sort((a, b) => (b.risk_score || 0) - (a.risk_score || 0));

  sorted.forEach(o => {
    const score = o.risk_score || 0;
    const cls = score >= 80 ? "critical" : (score >= 40 ? "risk" : "ok");

    const div = document.createElement("div");
    div.className = "organ-card";
    div.innerHTML = `
      <h3>${o.organ_id} <span class="badge ${cls}">${o.priority || "LOW"}</span></h3>
      <div class="meta">
        <div>Risk score: ${score}/100</div>
        <div>Classification: ${o.classification || "-"}</div>
        <div>Blockers: ${o.blockers || 0}</div>
        <div>Missing contract: ${!!o.missing_contract}</div>
        <div>Missing self-report: ${!!o.missing_self_report}</div>
        <div>Missing receipts: ${!!o.missing_receipts}</div>
        <div>Utility declared: ${!!o.utility_declared}</div>
        <div>Utility script-backed: ${!!o.utility_script_backed}</div>
      </div>
    `;
    root.appendChild(div);
  });
}

let SELECTED_ORGAN_ID = null;

function findOrganBundle(organId) {
  const gaps = DATA?.gaps?.organs || [];
  const utility = DATA?.utility?.organs || [];
  const risk = DATA?.useful?.organ_risk || [];
  const priority = DATA?.useful?.priority_fixes || [];
  const major = DATA?.gaps?.major_gaps || [];

  const g = gaps.find(o => o.organ_id === organId) || {};
  const u = utility.find(o => o.organ_id === organId) || {};
  const r = risk.find(o => o.organ_id === organId) || {};
  const p = priority.filter(x => x.organ === organId);
  const mg = major.filter(x => x.organ === organId);

  return { g, u, r, p, mg };
}

function yesNo(v) {
  return v ? "YES" : "NO";
}

function fillList(id, items, emptyText = "none") {
  const root = $(id);
  if (!root) return;
  root.innerHTML = "";

  if (!items || !items.length) {
    const div = document.createElement("div");
    div.textContent = emptyText;
    root.appendChild(div);
    return;
  }

  for (const item of items) {
    const div = document.createElement("div");
    div.textContent = typeof item === "string" ? item : JSON.stringify(item);
    root.appendChild(div);
  }
}

function openOrganDrawer(organId) {
  SELECTED_ORGAN_ID = organId;

  const { g, u, r, p, mg } = findOrganBundle(organId);

  $("drawerOrganName").textContent = organId;
  $("drawerRiskScore").textContent = r.risk_score ?? 0;
  $("drawerClass").textContent = g.classification || r.classification || "-";
  $("drawerPriority").textContent = r.priority || "-";
  $("drawerBlockers").textContent = (g.blockers || []).length;

  $("drawerContract").textContent = yesNo(g.organ_contract_exists);
  $("drawerSelfReport").textContent = yesNo(g.self_report_exists);
  $("drawerReceipts").textContent = yesNo(g.receipts_exists);
  $("drawerUtilityDeclared").textContent = yesNo(u.utility_declared);
  $("drawerUtilityBacked").textContent = yesNo(u.script_backed);

  const actions = p.map(x => `${x.action} — ${x.reason}`);
  const blockers = [
    ...(g.blockers || []),
    ...mg.map(x => x.gap)
  ];

  fillList("drawerActions", actions, "No priority actions generated.");
  fillList("drawerBlockerList", blockers, "No blockers for this organ.");
  fillList("drawerWhyNotCanon", g.why_not_canon || [], "No why_not_canon entries.");

  $("drawerPath").textContent = g.path || `E:\\IMPERIUM\\ORGANS\\${organId}`;

  $("organDrawer").classList.remove("hidden");
}

function closeOrganDrawer() {
  $("organDrawer")?.classList.add("hidden");
}

async function openSelectedOrganFolder() {
  if (!SELECTED_ORGAN_ID) return;
  await fetch("/api/open-organ?organ=" + encodeURIComponent(SELECTED_ORGAN_ID), { method: "POST" });
}

async function copySelectedOrganPath() {
  const txt = $("drawerPath")?.textContent || "";
  try {
    await navigator.clipboard.writeText(txt);
    showToast("Path copied");
  } catch {
    showToast(txt);
  }
}

/* Override earlier renderOrganCards with clickable detail cards */
function renderOrganCards(organs) {
  const root = $("organCards");
  root.innerHTML = "";

  for (const o of organs) {
    const blockers = (o.blockers || []).length;
    const div = document.createElement("div");
    div.className = "organ-card";
    div.dataset.organ = o.organ_id;
    div.innerHTML = `
      <h3>${o.organ_id} <span class="badge ${severityClass(blockers)}">${o.classification}</span></h3>
      <div class="meta">
        <div>Path: ${o.path}</div>
        <div>Blockers: ${blockers}</div>
        <div>Contract: ${boolMark(o.organ_contract_exists)}</div>
        <div>Self-report: ${boolMark(o.self_report_exists)}</div>
        <div>Receipts: ${boolMark(o.receipts_exists)}</div>
        <div>Why not canon: ${(o.why_not_canon || []).join("; ") || "-"}</div>
        <div><b>Click to inspect organ detail</b></div>
      </div>
    `;
    div.addEventListener("click", () => openOrganDrawer(o.organ_id));
    root.appendChild(div);
  }
}

/* Override earlier top table rows so they are clickable too */
function renderTopOrganRows(organs) {
  const tb = $("topOrganRows");
  tb.innerHTML = "";
  const sorted = [...organs].sort((a,b) => (b.blockers || []).length - (a.blockers || []).length).slice(0, 7);

  for (const o of sorted) {
    const blockers = (o.blockers || []).length;
    const tr = document.createElement("tr");
    const topGap = (o.blockers || [])[0] || "-";
    tr.style.cursor = "pointer";
    tr.innerHTML = `
      <td>${o.organ_id}</td>
      <td><span class="badge ${severityClass(blockers)}">${o.classification}</span></td>
      <td>${blockers}</td>
      <td>${boolMark(o.organ_contract_exists)}</td>
      <td>${boolMark(o.self_report_exists)}</td>
      <td>${boolMark(o.receipts_exists)}</td>
      <td>${topGap}</td>
    `;
    tr.addEventListener("click", () => openOrganDrawer(o.organ_id));
    tb.appendChild(tr);
  }
}

setTimeout(() => {
  $("drawerClose")?.addEventListener("click", closeOrganDrawer);
  $("drawerOpenFolder")?.addEventListener("click", openSelectedOrganFolder);
  $("drawerCopyPath")?.addEventListener("click", copySelectedOrganPath);
}, 0);

// v0.8 smoother animation override
let __bgLastW = 0;
let __bgLastH = 0;
let __sparkLastW = 0;
let __sparkLastH = 0;
let __smoothFps = 60;
let __bgPhase = 0;

function resizeCanvasIfNeeded(canvas, cssW, cssH) {
  const dpr = Math.min(window.devicePixelRatio || 1, 2);
  const w = Math.max(1, Math.floor(cssW * dpr));
  const h = Math.max(1, Math.floor(cssH * dpr));
  if (canvas.width !== w || canvas.height !== h) {
    canvas.width = w;
    canvas.height = h;
  }
  const ctx = canvas.getContext("2d");
  ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
  return ctx;
}

function animateBg(now) {
  const canvas = $("bgCanvas");
  if (!canvas) return;

  const W = window.innerWidth;
  const H = window.innerHeight;
  const ctx = resizeCanvasIfNeeded(canvas, W, H);

  ctx.clearRect(0, 0, W, H);

  __bgPhase += 0.0065;

  const grad = ctx.createRadialGradient(W * 0.68, H * 0.18, 20, W * 0.68, H * 0.18, Math.max(W, H) * 0.72);
  grad.addColorStop(0, "rgba(180,92,255,0.105)");
  grad.addColorStop(0.42, "rgba(83,167,255,0.038)");
  grad.addColorStop(1, "rgba(0,0,0,0)");
  ctx.fillStyle = grad;
  ctx.fillRect(0, 0, W, H);

  ctx.save();
  ctx.globalCompositeOperation = "lighter";

  for (let wave = 0; wave < 3; wave++) {
    ctx.beginPath();

    const yBase = H - 108 - wave * 24;
    const amp1 = 9 + wave * 2;
    const amp2 = 3 + wave;

    for (let x = 0; x <= W; x += 9) {
      const y =
        yBase +
        Math.sin(x * 0.0085 + __bgPhase + wave * 1.1) * amp1 +
        Math.sin(x * 0.021 + __bgPhase * 0.62 + wave) * amp2;

      if (x === 0) ctx.moveTo(x, y);
      else ctx.lineTo(x, y);
    }

    ctx.strokeStyle = `rgba(180,92,255,${0.10 + wave * 0.035})`;
    ctx.lineWidth = 1.15;
    ctx.shadowBlur = 11;
    ctx.shadowColor = "rgba(180,92,255,0.30)";
    ctx.stroke();
  }

  for (let i = 0; i < 34; i++) {
    const x = ((Math.sin(i * 58.13 + now * 0.00012) + 1) / 2) * W;
    const y = ((Math.cos(i * 37.91 + now * 0.00016) + 1) / 2) * H;
    const a = 0.045 + (Math.sin(now * 0.0009 + i) + 1) * 0.035;

    ctx.fillStyle = `rgba(255,79,216,${a})`;
    ctx.beginPath();
    ctx.arc(x, y, 1.05, 0, Math.PI * 2);
    ctx.fill();
  }

  ctx.restore();

  const dt = Math.max(1, now - lastFrame);
  lastFrame = now;
  const instant = 1000 / dt;
  __smoothFps = __smoothFps * 0.9 + instant * 0.1;

  const fpsEl = $("fpsValue");
  if (fpsEl) fpsEl.textContent = Math.round(Math.min(99, __smoothFps));

  requestAnimationFrame(animateBg);
}

function animateSpark(now) {
  const canvas = $("sparkCanvas");
  if (!canvas) return;

  const r = canvas.getBoundingClientRect();
  const ctx = resizeCanvasIfNeeded(canvas, r.width, r.height);

  ctx.clearRect(0, 0, r.width, r.height);

  const gradient = ctx.createLinearGradient(0, 0, r.width, 0);
  gradient.addColorStop(0, "rgba(180,92,255,0.05)");
  gradient.addColorStop(0.45, "rgba(180,92,255,0.72)");
  gradient.addColorStop(1, "rgba(83,167,255,0.12)");

  ctx.beginPath();

  for (let x = 0; x < r.width; x += 7) {
    const y =
      r.height / 2 +
      Math.sin(x * 0.028 + now * 0.0021) * 4.6 +
      Math.sin(x * 0.095 + now * 0.0011) * 1.7;

    if (x === 0) ctx.moveTo(x, y);
    else ctx.lineTo(x, y);
  }

  ctx.strokeStyle = gradient;
  ctx.lineWidth = 1.35;
  ctx.shadowBlur = 8;
  ctx.shadowColor = "rgba(180,92,255,0.38)";
  ctx.stroke();

  requestAnimationFrame(animateSpark);
}


// v0.8 laws page repair: robust Codex/Law extraction
function collectLawObjects(node, acc, depth = 0) {
  if (!node || depth > 8) return;

  if (Array.isArray(node)) {
    for (const item of node) collectLawObjects(item, acc, depth + 1);
    return;
  }

  if (typeof node !== "object") return;

  const hasLawShape =
    node.law_id ||
    node.id?.toString?.().match?.(/^LAW-\d+/) ||
    (node.title && (node.severity || node.violation_verdict || node.enforcement_status));

  if (hasLawShape) {
    const copy = { ...node };
    if (!copy.law_id && copy.id) copy.law_id = copy.id;
    acc.push(copy);
    return;
  }

  for (const [key, value] of Object.entries(node)) {
    if (/^LAW-\d+/i.test(key) && value && typeof value === "object" && !Array.isArray(value)) {
      acc.push({ law_id: key, ...value });
    } else {
      collectLawObjects(value, acc, depth + 1);
    }
  }
}

function getLawArray(laws) {
  const acc = [];

  collectLawObjects(laws, acc);

  if (window.DATA) {
    collectLawObjects(window.DATA.mandatory_laws, acc);
    collectLawObjects(window.DATA.enforcement, acc);
  }

  const seen = new Set();
  const clean = [];

  for (const law of acc) {
    const id = law.law_id || law.id || law.title || JSON.stringify(law).slice(0, 80);
    if (seen.has(id)) continue;
    seen.add(id);

    clean.push({
      law_id: law.law_id || law.id || "LAW-UNKNOWN",
      title: law.title || law.name || law.law_title || "",
      severity: law.severity || law.level || "-",
      status: law.status || "-",
      enforcement_status: law.enforcement_status || law.enforcement || law.current_enforcement_status || "UNKNOWN",
      violation_verdict: law.violation_verdict || law.verdict || "-",
      source_document_path: law.source_document_path || law.source_path || law.source || "",
      applies_to: law.applies_to || [],
      required_evidence: law.required_evidence || [],
      enforcement_points: law.enforcement_points || []
    });
  }

  clean.sort((a, b) => String(a.law_id).localeCompare(String(b.law_id)));
  return clean;
}

function renderLawList(laws) {
  const arr = getLawArray(laws);
  const label = $("lawCountLabel");
  if (label) label.textContent = `${arr.length} laws`;

  const root = $("lawList");
  if (!root) return;

  root.innerHTML = "";

  const paths = DATA?.paths || {};
  const intro = document.createElement("div");
  intro.className = "law-card law-card-wide";
  intro.innerHTML = `
    <h3><span class="law-id">CODEX REGISTRY</span> - Doctrinarium law sources</h3>
    <div class="meta">
      <div>LAW_INDEX: ${paths.laws || "E:\\IMPERIUM\\ORGANS\\DOCTRINARIUM\\LAWS"}\\LAW_INDEX.json</div>
      <div>MANDATORY_LAWS: ${paths.laws || "E:\\IMPERIUM\\ORGANS\\DOCTRINARIUM\\LAWS"}\\MANDATORY_LAWS.json</div>
      <div>LAW_ENFORCEMENT_MAP: ${paths.laws || "E:\\IMPERIUM\\ORGANS\\DOCTRINARIUM\\LAWS"}\\LAW_ENFORCEMENT_MAP.json</div>
      <div>Meaning: laws are registered, but many are still not fully enforced by scripts/gates.</div>
    </div>
  `;
  root.appendChild(intro);

  if (!arr.length) {
    const empty = document.createElement("div");
    empty.className = "law-card";
    empty.innerHTML = `
      <h3><span class="law-id">NO LAWS DETECTED</span></h3>
      <div class="meta">
        <div>Frontend could not detect law objects in LAW_INDEX / MANDATORY_LAWS.</div>
        <div>Run Refresh, then restart v0.8 server.</div>
      </div>
    `;
    root.appendChild(empty);
    return;
  }

  for (const law of arr) {
    const enforcement = String(law.enforcement_status || "UNKNOWN");
    const isOpen = enforcement.includes("NOT") || enforcement.includes("UNKNOWN") || enforcement.includes("GAP");
    const badge = isOpen ? "critical" : "ok";

    const div = document.createElement("div");
    div.className = "law-card";
    div.innerHTML = `
      <h3><span class="law-id">${law.law_id}</span> - ${law.title || ""}</h3>
      <div class="meta">
        <div>Severity: ${law.severity}</div>
        <div>Status: ${law.status}</div>
        <div>Enforcement: <span class="badge ${badge}">${law.enforcement_status}</span></div>
        <div>Violation verdict: ${law.violation_verdict}</div>
        <div>Source: ${law.source_document_path || "-"}</div>
        <div>Enforcement points: ${(law.enforcement_points || []).join("; ") || "-"}</div>
      </div>
    `;
    root.appendChild(div);
  }
}


// v0.8 Playwright hardening: safe canvas drawing guards
function safeCanvasBox(canvas) {
  if (!canvas) return null;
  const rect = canvas.getBoundingClientRect();
  const w = Math.max(1, rect.width || canvas.clientWidth || 1);
  const h = Math.max(1, rect.height || canvas.clientHeight || 1);
  const dpr = Math.min(window.devicePixelRatio || 1, 2);

  const pxW = Math.max(1, Math.floor(w * dpr));
  const pxH = Math.max(1, Math.floor(h * dpr));

  if (canvas.width !== pxW || canvas.height !== pxH) {
    canvas.width = pxW;
    canvas.height = pxH;
  }

  const ctx = canvas.getContext("2d");
  ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
  return { ctx, w, h };
}

function drawStatusDonut(g) {
  const canvas = $("statusDonut");
  const box = safeCanvasBox(canvas);
  if (!box) return;

  const { ctx, w, h } = box;
  ctx.clearRect(0, 0, w, h);

  const summary = g?.classification_summary || {};
  const entries = Object.entries(summary).filter(([,v]) => Number(v) > 0);
  const total = entries.reduce((s, [,v]) => s + Number(v), 0) || 1;
  const colors = ["#45d483", "#ffd45a", "#ff4f6d", "#b45cff", "#53a7ff"];

  const cx = w / 2;
  const cy = h / 2;
  const rad = Math.max(6, Math.min(w, h) / 2 - 18);

  let start = -Math.PI / 2;

  if (!entries.length) {
    ctx.strokeStyle = "rgba(255,255,255,.12)";
    ctx.lineWidth = Math.max(4, Math.min(22, rad * 0.28));
    ctx.beginPath();
    ctx.arc(cx, cy, rad, 0, Math.PI * 2);
    ctx.stroke();
  } else {
    entries.forEach(([key, val], i) => {
      const a = (Number(val) / total) * Math.PI * 2;
      ctx.beginPath();
      ctx.arc(cx, cy, rad, start, start + a);
      ctx.lineWidth = Math.max(4, Math.min(28, rad * 0.30));
      ctx.strokeStyle = colors[i % colors.length];
      ctx.stroke();
      start += a;
    });
  }

  ctx.fillStyle = "#edf2ff";
  ctx.font = "bold 24px Segoe UI";
  ctx.textAlign = "center";
  ctx.fillText(String(total), cx, cy);
  ctx.font = "11px Segoe UI";
  ctx.fillStyle = "#9ca8c3";
  ctx.fillText("TOTAL", cx, cy + 20);

  const legend = $("statusLegend");
  if (legend) {
    legend.innerHTML = "";
    entries.forEach(([key,val], i) => {
      const div = document.createElement("div");
      div.innerHTML = `<span><i style="background:${colors[i % colors.length]}"></i>${key}</span><b>${val}</b>`;
      legend.appendChild(div);
    });
  }
}

function drawRadar(g) {
  const canvas = $("radarCanvas");
  const box = safeCanvasBox(canvas);
  if (!box) return;

  const { ctx, w, h } = box;
  ctx.clearRect(0, 0, w, h);

  const labels = ["FOLDER", "UNKNOWN", "SCAFFOLD", "CONTRACT", "GAPS"];
  const summary = g?.classification_summary || {};
  const vals = [
    summary.FOLDER_ONLY || 0,
    summary.UNKNOWN || 0,
    summary.SCAFFOLD || 0,
    (g?.major_gaps || []).filter(x => String(x.gap).includes("ORGAN_CONTRACT")).length,
    Math.min(10, Math.round((g?.total_blockers_found || 0) / 6))
  ];

  const max = Math.max(1, ...vals);
  const cx = w / 2;
  const cy = h / 2 + 10;
  const rad = Math.max(12, Math.min(w, h) / 2 - 35);

  ctx.strokeStyle = "rgba(255,255,255,.12)";
  for (let layer = 1; layer <= 4; layer++) {
    ctx.beginPath();
    for (let i = 0; i < labels.length; i++) {
      const a = -Math.PI / 2 + (i / labels.length) * Math.PI * 2;
      const rr = rad * layer / 4;
      const x = cx + Math.cos(a) * rr;
      const y = cy + Math.sin(a) * rr;
      if (i === 0) ctx.moveTo(x, y);
      else ctx.lineTo(x, y);
    }
    ctx.closePath();
    ctx.stroke();
  }

  ctx.beginPath();
  vals.forEach((v, i) => {
    const a = -Math.PI / 2 + (i / labels.length) * Math.PI * 2;
    const rr = rad * v / max;
    const x = cx + Math.cos(a) * rr;
    const y = cy + Math.sin(a) * rr;
    if (i === 0) ctx.moveTo(x, y);
    else ctx.lineTo(x, y);
  });
  ctx.closePath();
  ctx.fillStyle = "rgba(180,92,255,.35)";
  ctx.fill();
  ctx.strokeStyle = "#ff4fd8";
  ctx.lineWidth = 2;
  ctx.stroke();

  ctx.fillStyle = "#9ca8c3";
  ctx.font = "11px Segoe UI";
  ctx.textAlign = "center";
  labels.forEach((label, i) => {
    const a = -Math.PI / 2 + (i / labels.length) * Math.PI * 2;
    ctx.fillText(label, cx + Math.cos(a) * (rad + 22), cy + Math.sin(a) * (rad + 22));
  });
}

function drawTrend() {
  const canvas = $("trendCanvas");
  const box = safeCanvasBox(canvas);
  if (!box) return;

  const { ctx, w, h } = box;
  ctx.clearRect(0, 0, w, h);

  const pts = Array.from({ length: 28 }, (_, i) => {
    return 0.45 + Math.sin(i * 0.55) * 0.12 + Math.random() * 0.08;
  });

  ctx.beginPath();
  pts.forEach((p, i) => {
    const x = (i / (pts.length - 1)) * w;
    const y = h - p * h;
    if (i === 0) ctx.moveTo(x, y);
    else ctx.lineTo(x, y);
  });

  ctx.strokeStyle = "#b45cff";
  ctx.lineWidth = 2;
  ctx.stroke();

  const grad = ctx.createLinearGradient(0, 0, 0, h);
  grad.addColorStop(0, "rgba(180,92,255,.45)");
  grad.addColorStop(1, "rgba(180,92,255,0)");
  ctx.lineTo(w, h);
  ctx.lineTo(0, h);
  ctx.closePath();
  ctx.fillStyle = grad;
  ctx.fill();
}
