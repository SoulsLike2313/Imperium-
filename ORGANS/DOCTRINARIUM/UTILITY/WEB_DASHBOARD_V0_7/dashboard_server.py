...
WEB = DOC / "UTILITY" / "WEB_DASHBOARD_V0_7"
REPORTS = DOC / "REPORTS"
STATUS = DOC / "STATUS"
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if parsed.path == "/api/open-organ":
            qs = parse_qs(parsed.query)
            organ = (qs.get("organ") or [""])[0].strip()
            if not organ:
                self.send_json({"ok": False, "error": "Missing organ"}, 400)
                return

            safe = organ.replace("\\", "").replace("/", "").replace("..", "")
            p = ROOT / "ORGANS" / safe

            if not p.exists() or not p.is_dir():
                self.send_json({"ok": False, "error": f"Organ path not found: {p}"}, 404)
                return
...
def main():
    ...
    print(f"Doctrinarium Dashboard v0.7 running at {url}")
    webbrowser.open(url)
