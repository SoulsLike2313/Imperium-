...
APP_NAME = "IMPERIUM Sanctum v0.28 — Command Dashboard"
...
class TransferWindow(tk.Toplevel):
    def __init__(self, master):
        super().__init__(master)
        self.title("Sanctum Transfer Control — PC ⇄ VM2")
        self.geometry("1120x820")
        self.configure(bg=COLORS["bg"])

        self.ssh_user_host = "vboxuser2@127.0.0.1"
        self.ssh_port = "2223"
        self.ssh_key = Path.home() / ".ssh" / "imperium_pc_to_vm2_ed25519_20260418"

        self.pc_prompt_outbox = IMPERIUM_ROOT / "OUTBOX" / "VM2_PROMPTS"
        self.pc_bundle_inbox = IMPERIUM_ROOT / "INBOX" / "VM2_BUNDLES"
        self.runtime_receipts = IMPERIUM_ROOT / ".imperium_runtime" / "transfer" / "receipts"

        self.vm2_workdrop = "/home/vboxuser2/IMPERIUM_PRIVATE/WORKDROP"
        self.vm2_bundle_outbox = "/home/vboxuser2/IMPERIUM_PRIVATE/OUTBOX_BUNDLES"

        self.route_status_var = tk.StringVar(value="Route: PC_WINDOWS → VM2_UBUNTU | unlocked")
        self.status_var = tk.StringVar(value="Transfer Control ready.")
    ...
    def _build_ui(self):
        ...
        tk.Label(
            left,
            text="Prompt file name",
            bg=COLORS["panel"],
            fg=COLORS["muted"],
            font=("Consolas", 9),
        ).pack(anchor="w", padx=12)

        self.prompt_name_var = tk.StringVar(value="TASK-VM2-PROMPT")
        tk.Entry(
            left,
            textvariable=self.prompt_name_var,
            bg=COLORS["panel2"],
            fg=COLORS["text"],
            insertbackground=COLORS["accent"],
            relief="flat",
        ...
        tk.Button(
            left,
            text="Send Prompt to VM2 + Auto Open",
        ...
        tk.Button(
            right,
            text="Refresh Remote Bundles",
            command=self.refresh_remote_bundles,
            bg=COLORS["panel3"],
            fg=COLORS["text"],
            activebackground=COLORS["select"],
            relief="flat",
        ...
        self.bundle_list.pack(fill=tk.BOTH, expand=True, padx=12, pady=(0, 8))

        tk.Button(
            right,
            text="Fetch Selected Bundle",
            command=self.fetch_selected_bundle,
            bg=COLORS["panel3"],
            fg=COLORS["text"],
            activebackground=COLORS["select"],
            relief="flat",
        ...
        tk.Button(
            right,
            text="Fetch Latest Bundle",
            command=self.fetch_latest_bundle,
            bg=COLORS["panel3"],
            fg=COLORS["text"],
            activebackground=COLORS["select"],
            relief="flat",
    ...
    def run_cmd(self, args, label, timeout=90):
        try:
            ...
            self.write_receipt(label, "PASS" if ok else "FAIL", {
                "command": args,
            ...
            self.write_receipt(label, "ERROR", {
                "command": args,
                "error": repr(e),
            })
            return False, None

    def write_receipt(self, action, status, details):
        self.runtime_receipts.mkdir(parents=True, exist_ok=True)
        payload = {
            "schema": "SANCTUM_TRANSFER_RECEIPT_V1",
            "created_at": datetime.now().isoformat(timespec="seconds"),
            "action": action,
            "status": status,
            "route": "PC_WINDOWS_TO_VM2_UBUNTU",
            "pc_prompt_outbox": str(self.pc_prompt_outbox),
            "pc_bundle_inbox": str(self.pc_bundle_inbox),
            "vm2_workdrop": self.vm2_workdrop,
            "vm2_bundle_outbox": self.vm2_bundle_outbox,
            "details": details,
        }
        receipt_path = self.runtime_receipts / f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_{action}.json"
        receipt_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
        return receipt_path

    def ssh_base(self):
        return ["ssh", "-i", str(self.ssh_key), "-p", self.ssh_port, self.ssh_user_host]

    def scp_base(self):
        return ["scp", "-i", str(self.ssh_key), "-P", self.ssh_port]

    def test_route(self):
        cmd = self.ssh_base() + [f"mkdir -p {self.vm2_workdrop} {self.vm2_bundle_outbox} && echo VM2_ROUTE_OK"]
        ok, result = self.run_cmd(cmd, "test_route")
        if ok:
            self.status_var.set("Route test PASS: PC → VM2 reachable.")
            messagebox.showinfo("Transfer route", "Route test PASS.")
        else:
            stderr = result.stderr if result else "unknown error"
            self.status_var.set("Route test FAIL.")
            messagebox.showerror("Transfer route", stderr)

    def lock_route(self):
        ok, _ = self.run_cmd(
            self.ssh_base() + [f"test -d {self.vm2_workdrop} && test -d {self.vm2_bundle_outbox} && echo LOCK_OK"],
            "lock_route",
        )
        if not ok:
            messagebox.showerror("Lock route", "Cannot lock route: route test failed.")
            return

        lock_path = IMPERIUM_ROOT / ".imperium_runtime" / "transfer" / "VM2_ROUTE_LOCK.json"
        lock_path.parent.mkdir(parents=True, exist_ok=True)
        lock_payload = {
            "route_id": "PC_WINDOWS_TO_VM2_UBUNTU",
            "locked_at": datetime.now().isoformat(timespec="seconds"),
            "ssh_user_host": self.ssh_user_host,
            "ssh_port": self.ssh_port,
            "ssh_key_path": str(self.ssh_key),
            "pc_prompt_outbox": str(self.pc_prompt_outbox),
            "vm2_workdrop": self.vm2_workdrop,
            "vm2_bundle_outbox": self.vm2_bundle_outbox,
            "pc_bundle_inbox": str(self.pc_bundle_inbox),
            "secret_policy": "Key path only. Private key material is not stored in Git.",
        }
        lock_path.write_text(json.dumps(lock_payload, ensure_ascii=False, indent=2), encoding="utf-8")
        self.route_status_var.set("Route: PC_WINDOWS → VM2_UBUNTU | 🔒 locked")
        self.write_receipt("lock_route", "PASS", {"lock_path": str(lock_path)})
        messagebox.showinfo("Lock route", f"Route locked:\n{lock_path}")

    def send_prompt(self):
        self.pc_prompt_outbox.mkdir(parents=True, exist_ok=True)
        name = self.safe_name(self.prompt_name_var.get())
        local_file = self.pc_prompt_outbox / name
        remote_file = f"{self.vm2_workdrop}/{name}"

        prompt = self.prompt_text.get("1.0", tk.END).strip()
        if not prompt:
            messagebox.showwarning("Send prompt", "Prompt is empty.")
            return

        local_file.write_text(prompt + "\n", encoding="utf-8")

        ok, result = self.run_cmd(self.ssh_base() + [f"mkdir -p {self.vm2_workdrop}"], "send_prompt_mkdir")
        if not ok:
            messagebox.showerror("Send prompt", result.stderr if result else "mkdir failed")
            return

        ok, result = self.run_cmd(self.scp_base() + [str(local_file), f"{self.ssh_user_host}:{remote_file}"], "send_prompt_scp")
        if not ok:
            messagebox.showerror("Send prompt", result.stderr if result else "scp failed")
            return
        ...
        ok, result = self.run_cmd(self.ssh_base() + [open_cmd], "send_prompt_auto_open")

        self.write_receipt("send_prompt", "PASS" if ok else "PASS_WITH_OPEN_WARNING", {
            "local_file": str(local_file),
            "remote_file": remote_file,
            "auto_open_ok": ok,
        })

        self.status_var.set(f"Prompt sent: {remote_file}")
        messagebox.showinfo("Send prompt", f"Prompt sent to VM2:\n{remote_file}")

    def refresh_remote_bundles(self):
        self.bundle_list.delete(0, tk.END)
        cmd = self.ssh_base() + [f"ls -t {self.vm2_bundle_outbox}/*.zip 2>/dev/null | head -50"]
        ok, result = self.run_cmd(cmd, "list_remote_bundles")
        if not ok:
            self.status_var.set("No remote bundles or route error.")
            return
        lines = [line.strip() for line in result.stdout.splitlines() if line.strip()]
        for line in lines:
            self.bundle_list.insert(tk.END, line)
        self.status_var.set(f"Remote bundles listed: {len(lines)}")

    def selected_bundle_path(self):
        sel = self.bundle_list.curselection()
        if not sel:
            return None
        return self.bundle_list.get(sel[0]).strip()

    def fetch_latest_bundle(self):
        self.refresh_remote_bundles()
        if self.bundle_list.size() == 0:
            messagebox.showwarning("Fetch bundle", "No remote bundles found.")
            return
        self.bundle_list.selection_set(0)
        self.fetch_selected_bundle()

    def fetch_selected_bundle(self):
        remote_bundle = self.selected_bundle_path()
        if not remote_bundle:
            messagebox.showwarning("Fetch bundle", "Select bundle first.")
            return

        self.pc_bundle_inbox.mkdir(parents=True, exist_ok=True)

        ok, result = self.run_cmd(
            self.scp_base() + [f"{self.ssh_user_host}:{remote_bundle}", str(self.pc_bundle_inbox) + "\\"],
            "fetch_bundle_zip",
            timeout=180,
        )
        if not ok:
            messagebox.showerror("Fetch bundle", result.stderr if result else "scp failed")
            return

        self.run_cmd(
            self.scp_base() + [f"{self.ssh_user_host}:{remote_bundle}.sha256", str(self.pc_bundle_inbox) + "\\"],
            "fetch_bundle_sha256_optional",
            timeout=60,
        )

        self.write_receipt("fetch_bundle", "PASS", {
            "remote_bundle": remote_bundle,
            "pc_bundle_inbox": str(self.pc_bundle_inbox),
        })

        self.status_var.set(f"Fetched bundle: {remote_bundle}")
        messagebox.showinfo("Fetch bundle", f"Fetched:\n{remote_bundle}\n\nTo:\n{self.pc_bundle_inbox}")
...
class SanctumApp(tk.Tk):
    def __init__(self):
        ...
        self.task_cache = {}
        self.tick = 0
        # v0.28: stable dashboard mode. Continuous canvas redraw causes visible flicker.
    ...
    def _build_ui(self):
        ...
        make_btn("Transfer Control", lambda: TransferWindow(self)).pack(side=tk.LEFT, padx=4)
        ...
        self.status_var = tk.StringVar(
            value="Status: SANCTUM_CLIENT_SHELL_ONLY | UNIFIED_PLANET_MAP_V0_28 | FILES_ARE_TRUTH"
        )
        tk.Label(
            bottom,
            textvariable=self.status_var,
    ...
    def animate(self):
        # v0.28: continuous redraw is disabled by default to prevent hard flicker.
        # The map is redrawn on task selection, refresh, hover enter/leave, and manual events.
        self.tick += 1
        if getattr(self, "animation_enabled", False) and self.current_model:
            self.draw_unified_map(self.current_model)
...
if __name__ == "__main__":
    app = SanctumApp()
    # v0.28: dashboard launch must not modify tracked SANCTUM_STATUS.json or README.md
