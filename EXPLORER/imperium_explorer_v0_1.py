...
APP_NAME = "IMPERIUM Explorer V0.1"

# Если файл лежит в E:\IMPERIUM\EXPLORER\, то корень IMPERIUM = parent.parent
...
def scan_direct_markers(path: Path) -> dict:
    """
    Быстрая проверка только прямых детей папки.
    ВАЖНО: здесь нет path.rglob("*"), чтобы Explorer не зависал на больших папках.
    """
    result = {
        "HAS_MANIFEST": False,
...
class ImperiumExplorer(tk.Tk):
    ...
    def _load_children(self, node_id: str):
        ...
        # Если уже загружено — не дублируем.
        if existing:
            first = existing[0]
