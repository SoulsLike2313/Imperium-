...
# ВАЖНО:
# ARCHIVE сейчас является холодным safety-хранилищем.
# Он не участвует в текущем кодовом/историческом рабочем процессе.
# Поэтому его нельзя сканировать рекурсивно.
ARCHIVE_DIR_NAMES = {
    "ARCHIVE",
    "_ARCHIVE",
    "IMPERIUM_ARCHIVE",
...
def is_archive_dir(path: Path) -> bool:
    return path.is_dir() and path.name.upper() in ARCHIVE_DIR_NAMES
...
def collect_archive_top_index(root: Path, max_items_per_archive: int = 80) -> list[dict]:
    """
    Находит archive-папки и собирает только верховой индекс.
    Рекурсивно внутрь archive не заходит.
    """
    archives = []

    # Ищем archive только обходом рабочей структуры с pruning.
    # Если archive найден — фиксируем direct children и не идём внутрь.
    for current_root, dirnames, filenames in os.walk(root):
        current_path = Path(current_root)

        # Убираем технический мусор из обхода.
        ...
        # Ключевая строка:
        # archive-папки удаляем из dirnames, чтобы os.walk не пошёл внутрь.
...
def scan_tree_excluding_archive(root: Path, max_nodes: int = 50000) -> dict:
    ...
    for current_root, dirnames, filenames in os.walk(root):
        ...
        # Фиксируем archive, но не идём внутрь.
        archive_dirs_here = [d for d in dirnames if d.upper() in ARCHIVE_DIR_NAMES]
        for archive_name in archive_dirs_here:
            archive_path = current_path / archive_name
...
def build_markdown_report(report: dict) -> str:
    lines = []
    lines.append("# IMPERIUM Explorer Truth Audit V0.2")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Read-only statement")
    lines.append("")
    for note in EXPECTED_READ_ONLY_NOTE:
        lines.append(f"- {note}")
    lines.append("")
    ...
    lines.append("- ARCHIVE currently does not participate in active working process.")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Archive top index")
    lines.append("")
    if report["archive_top_index"]:
        for archive in report["archive_top_index"]:
            lines.append(f"### `{archive['archive_path']}`")
            lines.append("")
            lines.append(f"- recursive_scan: `{archive['recursive_scan']}`")
            counts = archive["direct_counts"]
            lines.append(f"- direct_folders: `{counts.get('direct_folders')}`")
            lines.append(f"- direct_files: `{counts.get('direct_files')}`")
            lines.append(f"- direct_items_sample_count: `{archive['direct_items_sample_count']}`")
            lines.append("")
    else:
        lines.append("- No archive roots detected.")
    lines.append("")
    ...
    lines.append("")
    lines.append("This audit does not prove that the GUI displays every node correctly.")
    lines.append("It proves the filesystem truth snapshot that Explorer must match.")
    lines.append("ARCHIVE is intentionally indexed only at top level.")
    lines.append("Next step: screenshot pack must visually compare Explorer v0.4 with this report.")
    lines.append("")
    return "\n".join(lines)
...
def main():
    ...
    if not (EXPLORER_ROOT / "imperium_explorer_v0_4.py").exists():
        findings.append({
            "severity": "BLOCKER",
            "code": "EXPLORER_V0_4_MISSING",
            "message": "imperium_explorer_v0_4.py was not found.",
        })
    ...
    report = {
        "audit_name": "IMPERIUM_EXPLORER_TRUTH_AUDIT_V0_2",
        "run_id": run_id,
        "created_at_local": datetime.now().isoformat(timespec="seconds"),
        "imperium_root": str(IMPERIUM_ROOT),
        "explorer_root": str(EXPLORER_ROOT),
        "read_only_statement": EXPECTED_READ_ONLY_NOTE,
        "archive_policy": {
    ...
    print("Explorer truth audit complete.")
    print(f"Verdict: {verdict}")
    print("Archive recursive scan: DISABLED")
    print(f"Archive roots indexed: {len(archive_top_index)}")
