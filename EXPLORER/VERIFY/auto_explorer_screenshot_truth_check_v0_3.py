...
EXPLORER_SCRIPT = EXPLORER_ROOT / "imperium_explorer_v0_6.py"
...
def load_explorer_module():
    spec = importlib.util.spec_from_file_location("imperium_explorer_v0_6", EXPLORER_SCRIPT)
...
def build_report_md(report):
    lines = []
    lines.append("# IMPERIUM Explorer Auto Screenshot Truth Check V0.3")
    lines.append("")
    lines.append(f"RUN_ID: `{report['run_id']}`")
    lines.append(f"VERDICT: `{report['verdict']}`")
...
def main():
    ...
    report = {
        "audit_name": "IMPERIUM_EXPLORER_AUTO_SCREENSHOT_TRUTH_CHECK_V0_3",
        "run_id": run_id,
    ...
    print("Auto Screenshot Truth Check V0.3 complete.")
