...
EXPLORER_SCRIPT = EXPLORER_ROOT / "imperium_explorer_v0_5.py"
...
def load_explorer_module():
    spec = importlib.util.spec_from_file_location("imperium_explorer_v0_5", EXPLORER_SCRIPT)
...
def build_report_md(report):
    lines = []
    lines.append("# IMPERIUM Explorer Auto Screenshot Truth Check V0.2")
    lines.append("")
    lines.append(f"RUN_ID: `{report['run_id']}`")
    lines.append(f"VERDICT: `{report['verdict']}`")
...
def main():
    ...
    report = {
        "audit_name": "IMPERIUM_EXPLORER_AUTO_SCREENSHOT_TRUTH_CHECK_V0_2",
        "run_id": run_id,
    ...
    print("Auto screenshot truth check v0.2 complete.")
